...
(define (val exp)
  (cond ((vardef? exp) (caddr exp))
        ((procdef? exp) (list 'lambda (cdadr exp) (caddr exp)))
        (else (error "Bad define syntax" exp))))
...
;;;
(define test-exp '(lambda (n)
                    (define (iter a b cnt)
                      (if (zero? cnt)
                          a
                          (iter b (+ a b) (- cnt 1))))
                    (iter 0 1 n)))
(define b (lambda-body test-exp))
(define scanned (scan-out-defines b))

(define my-let '(let ((x 7)) (define y 5) (+ x y)))
(define my-let* '(let* ((x 2) (y (+ x 1))) (define z 4) (+ x y z)))
